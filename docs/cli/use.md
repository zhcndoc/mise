...
如果存在多个配置文件（例如同时存在 `mise.toml` 和 `mise.local.toml`），将使用优先级最低的文件（`mise.toml`）。
...
## 标志
- **`-e --env <ENV>`** — 创建或修改特定环境的配置文件，例如 .mise.&lt;env>.toml
- **`-f --force`** — 即使已安装，也强制重新安装
...
- **`--dry-run-code`** — 类似于 --dry-run，但如果有要进行的更改，则以代码 1 退出

  这对于脚本检查是否需要添加或移除工具很有用。
- **`--fuzzy`** — 将模糊版本保存到配置文件

  例如：`mise use --fuzzy node@20` 会将 20 作为版本保存
