---
description: "在 `[dotfiles]` 中添加或更新 dotfiles"
---

<!-- @generated by usage-cli from usage spec -->
# `mise bootstrap dotfiles add`

- **用法：** `mise bootstrap dotfiles add [FLAGS] [TARGET]…`
- **作用：** 修改状态
- **源代码：** [`src/cli/dotfiles/add.rs`](https://github.com/jdx/mise/blob/main/src/cli/dotfiles/add.rs)

在 `[dotfiles]` 中添加或更新 dotfiles

如果目标已被管理，则从实时目标更新其来源。否则创建一个 `[dotfiles]` 条目，并在 `dotfiles.root` 下初始化来源，除非提供了 `--source`。除非传入 `--no-apply`，否则会应用捕获的条目。使用 `--dry-run` 可预览来源捕获和配置写入，而不会实际进行这些更改。

## 参数
- **`[TARGET]…`** — 要添加或更新的目标

## 选项
- **`--changed`** — 更新以 copy 模式管理的每个已更改文件的来源
- **`-f --force`** — 不提示，覆盖现有来源
- **`-g --global`** — 写入全局配置
- **`-l --local`** — 写入本地配置，而不是全局配置
- **`-m --mode <MODE>`** — 要写入的 dotfile 模式
- **`-n --dry-run`** — 打印配置和来源更新，但不写入任何内容
- **`--no-apply`** — 添加条目但不应用
- **`-p --path <PATH>`** — 写入此配置文件或目录
- **`-s --source <PATH>`** — 用于单个目标的来源路径
- **`-y --yes`** — 跳过确认提示
- **`--prompt-secrets`** — 安全地提示输入缺失的 bootstrap secret
- **`-h --help`** — 打印帮助

## 示例

```
mise dot add ~/.zshrc
mise dot add --mode copy ~/.config/starship.toml
mise dot add --source dotfiles/gitconfig ~/.gitconfig
mise dot add --changed
```

<!-- generated reference navigation -->

## 相关文档

- [Dotfile 的所有权和模式](/dotfiles.html)。
- [`mise bootstrap dotfiles <SUBCOMMAND>`](/cli/bootstrap/dotfiles.html)。
- [全局选项和参数语法](/cli/#global-flags)。
