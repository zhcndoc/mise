...
和工作目录；原始命令和环境值不会存储在状态中。

1. 计算所有源文件的 blake3 哈希
2. 计算有效提供者命令的 blake3 哈希
3. 与上次成功运行时存储的哈希进行比较
4. 如果源文件或有效命令被添加、移除或更改，则提供者会被视为过期

这意味着：

- 如果修改了 `package-lock.json`，`node_modules/` 将被视为过期
- 如果 `node_modules/` 不存在，提供者始终会被视为过期
- 如果源文件不存在，提供者会被视为最新（无需执行任何操作）
- 首次运行时（没有存储的状态），提供者始终会被视为过期
- 命令哈希功能启用前创建的状态，会通过运行每个提供者一次来迁移

## 自动安装

当在提供程序上设置 `auto = true` 时，它会在以下命令之前自动运行：
