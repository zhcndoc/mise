...
- **`--include-task-tools`** — 同时安装当前作用域内任务所需的工具
  ...
  此操作会准备任务工具，但不会运行任务命令或依赖项。
  与 `--monorepo` 结合使用可包含每个已配置根目录中的任务。
- **`--minimum-release-age <MINIMUM_RELEASE_AGE>`** — 仅安装在此日期之前或早于此时长发布的版本
...
- **`--monorepo`** — 从每个 [monorepo].config_roots 配置根目录安装工具
  ...
  **环境变量：** `MISE_MONOREPO`
- **`--raw`** — 将 backend 安装命令的 stdin/stdout/stderr 直接连接到终端。隐含 `--jobs=1`
