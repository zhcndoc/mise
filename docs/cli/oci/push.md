---
description: "[实验性] 构建 OCI 镜像并将其推送到镜像仓库"
---
...
工具、版本、挂载点和文件所有者均与之前推送的镜像（或 `--cache-from`）匹配的工具层会直接复用，无需重新构建——这些工具甚至无需在本地安装。传入 `--no-cache` 可在不使用远程或本地层缓存的情况下重新构建工具层。
...
- **`--no-cache`** — 在不使用远程或本地层缓存的情况下重新构建工具层
