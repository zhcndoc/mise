...
升级到可用的最新版本，并提升 mise.toml 中的版本

例如，如果你的 mise.toml 中有 `node = "20.0.0"`，但可用的最新版本是 22.1.0，
则会安装 22.1.0，并在配置中设置 `node = "22.1.0"`。

它会保持之前的版本精度，因此如果之前是 `node = "20"`，它会将配置更改为 `node = "22"`。
